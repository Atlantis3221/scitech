    ...
    const childArr: any = React.Children.map(children, (child: any) => {
        if (child.type !== "div" && child.type !== "button") {
            return React.cloneElement(child, {
                ...child.props,
                disabled,
                className: child.props.className + ` z-0 placeholder-gray-500 text-black w-full leading-4 placeholder-opacity-100 ${disabled ? "bg-gray-30" : " bg-white"} outline-none transition-all duration-200 resize-none text-ui-text ${child.type === "textarea" && 'resize-y overflow-hidden'}`
            })
        }
        return child
    })
